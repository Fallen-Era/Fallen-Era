...
#if WITH_EDITOR
// The engine can't see WorldPositionOffset/PixelDepthOffset/Displacement through our custom root expression:
// MaterialMayModifyMeshPosition() is then false while the translated material does use them, so the shader map id
// misses shader types the compiled material expects (eg TShadowDepthPSCombined, asserts on 5.8).
// Wrap the root in an identity SetMaterialAttributes so they're all detected as connected.
static void MarkMeshPositionPropertiesConnected(UMaterial& Material)
{
	...
	static const TVoxelArray<TPair<FName, EMaterialProperty>> Properties
	{
		{ "WorldPositionOffset", MP_WorldPositionOffset },
		{ "PixelDepthOffset", MP_PixelDepthOffset },
	...
	for (const auto& It : Properties)
	{
		...
		{
			if (Break.Outputs[Index].OutputName == It.Key)
			{
				OutputIndex = Index;
				break;
			}
		}
		if (!ensure(OutputIndex != INDEX_NONE))
		{
			continue;
		}

		Set.AttributeSetTypes.Add(FMaterialAttributeDefinitionMap::GetID(It.Value));
		Set.Inputs.Emplace_GetRef().Connect(OutputIndex, &Break);
	}
	...
	UMaterialInstanceConstant& NewMaterialInstance)
{
	...
	MarkMeshPositionPropertiesConnected(NewMaterial);
